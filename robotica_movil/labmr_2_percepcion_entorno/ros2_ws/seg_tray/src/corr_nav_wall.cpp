...
private:
    ...
    {
        // Estimar pared izquierda (60° a 120°) y pared derecha (-120° a -60°)
        auto [ml, bl] = extractWall(msg,  M_PI / 3.0,  2.0 * M_PI / 3.0);
        auto [mr, br] = extractWall(msg, -2.0 * M_PI / 3.0, -M_PI / 3.0);
    ...
    void controlLoop()
    {
        if (!measured_data_) {
            ...
            // Guard: si alguna pared no fue detectada, parar
            if (!std::isfinite(b_left_) || !std::isfinite(b_right_)) {
                RCLCPP_WARN(this->get_logger(), "Pared no detectada (b_left=%.2f b_right=%.2f), esperando...", b_left_, b_right_);
                geometry_msgs::msg::Twist stop;
                cmd_vel_pub_->publish(stop);
                return;
            }

            // Pure Pursuit: el punto objetivo es el centro del pasillo a distancia L
            // Pared izq: y = m_left*x + b_left    Pared der: y = m_right*x + b_right
            // Centro en x=L: y_goal = (y_left(L) + y_right(L)) / 2
            double L = look_ahead_distance_;
            double y_goal = ((m_left_ + m_right_) / 2.0) * L + (b_left_ + b_right_) / 2.0;
            double curvature = 2.0 * y_goal / (L * L);
            double linear_velocity  = max_linear_speed_ * 0.5;
            double angular_velocity = std::clamp(curvature * linear_velocity, -max_angular_speed_, max_angular_speed_);

            RCLCPP_INFO(this->get_logger(),
                "b_L=%.2f b_R=%.2f y_goal=%.3f curv=%.4f v=%.2f w=%.2f",
                b_left_, b_right_, y_goal, curvature, linear_velocity, angular_velocity);
